...
Examine the temporal dynamics of messages in relation to public events.
Perform a basic analysis of the content of the messages.
This document accompanies a Shiny application that allows interactive exploration of the Enron email dataset. Below, we detail the steps of the analysis and provide scientific commentary based on statistical results. #Code for the Shiny Application and Load Data
...
Load the Enron data
rm(list = ls()) setwd(“C:/Users/TRUNG NGUYEN/Downloads”) load(“Enron (1).Rdata”)
...
Filter messages sent between 1999 and 2002.(because many emails have value in year 0001 and 2044 => this is an quality issue)
...
#Exploration of the Most Active Enron Employees I identify the most active employees with their roles based on the number of emails sent.
...
ggplot(head(active_employees, 10), aes(x = reorder(fullname, Email_count), y = Email_count, fill = status)) +
  ...
  labs(title = "Top 10 Active Employees",
       ...
       fill = "Status") +
  theme_minimal()
Scientific Commentary: - Based on the top 10 active employees who send emails, I found that only Kean Steven, Vice President, and Kaminski Vince, Manager, hold management positions among the most active email senders.
...
Scientific Commentary: - Within two years, the Company changed 4 CEOs, 3 Managing Directors, and 4 Presidents. This is a signal of the company’s instability during the period 1999-2001. - Moreover, there is a concern about the number of employees: There is only one in-house lawyer, but there are 13 traders in the company. - Another point is in-house lawyer has over 700 emails which is the lowest number of email by positon => The quick changing of the top level in the Company reflect on the issue about the performance of them. Moreover, I guess the BOD of the Company had not seek advice from the in-house lawyer in some business decision.

#Analysis of the Temporal Dynamics of Messages I examine email activity over time, particularly around significant public events related to the Enron scandal.
...
ggplot() +
  ...
  labs(title = "Emails Over Time with Significant Events",
       ...
       y = "Number of Emails Sent") +
  theme_minimal()
Scientific Commentary: - There are noticeable spikes in email activity around key events, such as the CEO’s resignation and the bankruptcy filing
...
analysis of email subjects to identify common themes and sentiments. Frequency Analysis of Words in Subjects
...
#Conclusion Through this analysis, we’ve gained insights into the communication patterns within Enron during a critical period. Key findings include:
...
References
Enron Email Dataset: Enron Email Dataset Tidytext Package: Text Mining with R Shiny Package: Building Web Applications in R
